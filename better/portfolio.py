...
from better.building import Building
...
class Portfolio:
    """Creates an instance of the Portfolio class using the portfolio name as the only input."""

    def __init__(self, name):
        self.name = name

    def read_raw_data_from_xlsx(self,
                                file_path: pathlib.Path) -> None:
        """Reads in Portfolio metadata and utility detail and stores them in the df_meta and df_detail properties."""

        # clean up the raw data and save it as a dataframe
        self.df_meta = pd.read_excel(
            file_path, sheet_name="Metadata", skiprows=[0, 1], usecols="A:I")
        self.df_detail = pd.read_excel(file_path,
                                       sheet_name="Utility",
                                       skiprows=2,
                                       usecols="A:G",
                                       parse_dates=[1, 2],)
        ...
        self.df_detail = self.df_detail[np.isfinite(
            self.df_detail['building_ID'])]

    def get_utility_by_building_id_and_energy_type(self,
                                                   # Should change building_id to a string throughout to make it more flexible
                                                   building_id: str | int,
                                                   energy_type: int):
        """
        Energy Type 1 is electricity and 2 is fossil fuel
        """

        # energy_type: 1 ~ electricity; 2 ~ fossil fuel
        df_temp = self.df_detail.loc[self.df_detail['building_ID']
                                     == building_id]
        df_temp = df_temp[['bill_start_dates', 'bill_end_dates', 'energy_type',
                           'energy_unit', 'energy_consumption', 'energy_cost']]
        if (energy_type == 1):
            df_temp = df_temp.loc[df_temp['energy_type']
        ...
        df_temp.loc[df_temp['energy_unit'] == 'MJ',
                    'energy_consumption'] *= Constants.MJ_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'GJ',
                    'energy_consumption'] *= Constants.GJ_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'MWh',
                    'energy_consumption'] *= Constants.MWH_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'Btu',
                    'energy_consumption'] *= Constants.Btu_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'MMBtu',
                    'energy_consumption'] *= Constants.MMBtu_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'Cubic Meters',
                    'energy_consumption'] *= Constants.M3_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'Therms',
                    'energy_consumption'] *= Constants.Therms_to_kWh
        df_temp.loc[df_temp['energy_unit'] == 'Decatherms',
                    'energy_consumption'] *= Constants.Decatherms_to_kWh

        # Format the dataframe to match the raw utility data frame
        df_temp = df_temp[['bill_start_dates', 'bill_end_dates',
    ...
    def get_building_info_by_id(self, building_ID):
        try:
            df_temp = self.df_meta.loc[self.df_meta['building_ID']
                                       == building_ID]
    ...
    def get_portfolio_raw_data_by_spaceType_and_utilityType(self,
                                                            space_type: Literal['Office'],
                                                            utility_type: int):
        """
        Consolidates building, utility, and currency data into a dictionary for a given space type and utility type

        Utility type 1 is electric. Utility type 2 is fossil fuel. Consider changing this input to a string literal.
        """
        ...
        df_temp_meta = df_temp_meta.drop_duplicates(
        ...
        # Add raw utility data into the dictionaries
        for i in df_temp_meta['building_ID']:
            if (utility_type == 1):
                ...
                utility_temp = Utility(
                    'electricity', df_temp_detail_utility)
            else:
                df_temp_detail_utility = (
                    self.get_utility_by_building_id_and_energy_type(i, 2))
                utility_temp = Utility(
                    'fossil fuel', df_temp_detail_utility)

            index = df_temp_meta[df_temp_meta['building_ID'] == i].index.tolist()[
                0]
    ...
    @staticmethod
    def generate_building_models(dict_raw_utility: dict,
        ...
        for bldg_id in v_building_ID:
            ...
            if (hasattr(utility_temp, "df_raw_data")):
                ...
                building_temp = Building(
                    bldg_id, bldg_name, bldg_address, bldg_type, bldg_area, currency)
                weather_temp = Weather(building_temp.coord)
                building_temp.add_utility(utility_temp)
                building_temp.add_weather(
                    weather_e=weather_temp, cached=use_cached_weather)
                ...
                if (has_fit):
                    ...
                    v_beta_base = np.append(
                        v_beta_base, building_temp.im_electricity.coeffs['base'])
                    v_beta_betc = np.append(
                        v_beta_betc, building_temp.im_electricity.coeffs['ccp'])
                    v_beta_beth = np.append(
                        v_beta_beth, building_temp.im_electricity.coeffs['hcp'])
    ...
    def generate_benchmark_stats(df_building_models):
        df_bench_stats = pd.DataFrame(
            columns=['beta_median', 'beta_standard_deviation'])
        df_bench_stats.index.name = "coefficient"
        median_BASE, std_BASE = Benchmark.generate_benchmark_stats(
            'BASE', df_building_models['beta_base'])
        median_CSL, std_CSL = Benchmark.generate_benchmark_stats(
            'CSL', df_building_models['beta_cdd'])
        median_CCP, std_CCP = Benchmark.generate_benchmark_stats(
            'CCP', df_building_models['beta_betc'])
        median_HSL, std_HSL = Benchmark.generate_benchmark_stats(
            'HSL', df_building_models['beta_hdd'])
        median_HCP, std_HCP = Benchmark.generate_benchmark_stats(
            'HCP', df_building_models['beta_beth'])
        df_bench_stats.at['beta_base', 'beta_median'], df_bench_stats.at['beta_base',
                                                                         'beta_standard_deviation'] = median_BASE, std_BASE
        df_bench_stats.at['beta_cdd', 'beta_median'], df_bench_stats.at['beta_cdd',
                                                                        'beta_standard_deviation'] = median_CSL, std_CSL
        df_bench_stats.at['beta_betc', 'beta_median'], df_bench_stats.at['beta_betc',
                                                                         'beta_standard_deviation'] = median_CCP, std_CCP
        df_bench_stats.at['beta_hdd', 'beta_median'], df_bench_stats.at['beta_hdd',
                                                                        'beta_standard_deviation'] = median_HSL, std_HSL
        df_bench_stats.at['beta_beth', 'beta_median'], df_bench_stats.at['beta_beth',
                                                                         'beta_standard_deviation'] = median_HCP, std_HCP
        return df_bench_stats

    @staticmethod
    def generate_benchmark_stats_wrapper(dict_raw_utility: dict,
                                         use_cached_weather: bool):
        df_building_models = Portfolio.generate_building_models(
            dict_raw_utility, use_cached_weather)
    ...
    def prepare_portfolio_report_data(self, v_single_buildings, report_path, save_portfolio_results=True):
        ...
        for single_building in v_single_buildings:
            if single_building != None:
                ...
                rpt_path = (str(single_building.bldg_id) + '_' + single_building.bldg_address +
                            '_' + single_building.bldg_name + '_report.html').replace(' ', '_')
                ...
                if hasattr(single_building, "recent_annual_electricity_kWh"):
                    ...
                    v_single_annual_consumption_e.append(
                        single_building.recent_annual_electricity_kWh)
                else:
                ...
                if hasattr(single_building, "recent_annual_fossil_fuel_kWh"):
                    ...
                    v_single_annual_consumption_f.append(
                        single_building.recent_annual_fossil_fuel_kWh)
                else:
                ...
                if hasattr(single_building, "recent_annual_electricity_cost"):
                    ...
                    v_single_annual_cost_e.append(
                        single_building.recent_annual_electricity_cost)
                else:
                ...
                if hasattr(single_building, "recent_annual_fossil_fuel_cost"):
                    ...
                    v_single_annual_cost_f.append(
                        single_building.recent_annual_fossil_fuel_cost)
                else:
                    v_single_annual_cost_f.append("NA")

                if hasattr(single_building, "recent_annual_electricity_EUI"):
                    v_single_eui_e.append(
                        int(single_building.recent_annual_electricity_EUI))
                else:
                    v_single_eui_e.append('NA')

                if hasattr(single_building, "recent_annual_fossil_fuel_EUI"):
                    v_single_eui_f.append(
                        int(single_building.recent_annual_fossil_fuel_EUI))
                else:
                ...
                if hasattr(single_building, "total_cost_savings"):
                    ...
                    v_single_cost_savings.append(
                        int(single_building.total_cost_savings))
                else:
                    v_single_cost_savings.append("NA")

                if hasattr(single_building, "total_energy_savings_pct"):
                    v_single_cost_savings_pct.append(
                        int(single_building.total_energy_savings_pct))
                else:
        ...
        if total_area > 0:
            self.portfolio_eui_e = round(
                self.total_annual_consumption_e/total_area, 0)
        if total_area > 0:
            self.portfolio_eui_f = round(
                self.total_annual_consumption_f/total_area, 0)
        ...
        if save_portfolio_results:
            s_path = os.path.dirname(
                os.path.dirname(os.path.realpath(__file__)))
            output_path = s_path + '/outputs/'
            self.df_bldg_summary.to_csv(
                output_path + 'portfolio_results.csv', index=False)
...
if __name__ == "__main__":
    ...
    # dict_utility_e = p.get_portfolio_raw_data_by_spaceType_and_utilityType(
    #     'Hotel_酒店', 1)
    # dict_utility_f = p.get_portfolio_raw_data_by_spaceType_and_utilityType(
    #     'Hotel_酒店', 2)
    ...
    # import datetime
    # df_models_e.to_csv('C:/Users/Han/Documents/GitHub/CERC/CERC-BEE-Benchmarking-Tool/data/' +
    #                    datetime.datetime.today().strftime('%Y-%m-%d') + 'models_e.csv')
    # df_models_f.to_csv('C:/Users/Han/Documents/GitHub/CERC/CERC-BEE-Benchmarking-Tool/data/' +
    #                    datetime.datetime.today().strftime('%Y-%m-%d') + 'models_f.csv')
    # df_bench_coeffs_e.to_csv('C:/Users/Han/Documents/GitHub/CERC/CERC-BEE-Benchmarking-Tool/data/' +
    #                          datetime.datetime.today().strftime('%Y-%m-%d') + 'bench_stats_e.csv')
    # df_bench_coeffs_f.to_csv('C:/Users/Han/Documents/GitHub/CERC/CERC-BEE-Benchmarking-Tool/data/' +
    #                          datetime.datetime.today().strftime('%Y-%m-%d') + 'bench_stats_f.csv')
